import { BackTop, Button, Divider, Image, List, message, Modal } from "antd";
...
/**
 * get Description based on time and location
 *
 * @param {*} time start time of the event
 * @param {*} loc location of the event
 * @returns event description display
 */
  ...
  const fetchData = () => {
    ...
    axios(opt)
      ...
      .catch((err) => {
        console.log(err);
        message.error(
          "Cannot load events at this time, please try again later"
        );
      });
  };
